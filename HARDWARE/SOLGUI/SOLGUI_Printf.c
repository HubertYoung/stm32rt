...
FontInfo SOLGUI_SwitchFont(u8 mode)
{
	...
	case 0:
	case R6X8:
		return (_fontInfo_6x8);
#if FONT4X6_EN == 1
	case F4X6:
	case R4X6:
		return (_fontInfo_4x6);
#endif
#if FONT8X8_EN == 1
	case F8X8:
	case R8X8:
		return (_fontInfo_8x8);
#endif
#if FONT8X10_EN == 1
	case F8X10:
	case R8X10:
		return (_fontInfo_8x10);
#endif
	default:
		return (_fontInfo_6x8);
	}
...
void __SOLGUI_printf(u32 x, u32 y, u8 mode, const u8 *str, va_list arp) //SOLGUI内部使用的屏幕printf底层
{
	...
	for (cc = res = 0; cc != -1; res += cc) //解析格式化字符串，且输出
	{
		...
		{ //%0，0填充
			f = 1;
			c = *str++; //f0填充标记位置1，读取下一个字符
		}
		else if (c == '-')
		{ //%-，左对齐（左边填空格）
			f = 2;
			c = *str++; //f左对齐标记位置1，读取下一个字符
		}
		else if (c == '.')
		{ //%.3f表示留3位小数
			fl = 1;
		...
		{					   //数字解析成字符
			...
			v /= r;			   //递进
			if (d > 9)
				d += (c == 'x') ? 0x27 : 0x07;	   //判读，不是10进制而是16进制时，d欲先加一个间隔跳转至'A'(大写X)或'a'(小写X)
