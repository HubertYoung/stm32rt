...
void FIFO_EnQueue(u8 KeyCode)
{
	...
	if (++_key_cache.Write >= FIFOBUF_SIZE)
		_key_cache.Write = 0;
}
...
{
	u8 ret;
	if (_key_cache.Read == _key_cache.Write)
		return (FIFO_NONE); //FIFO空
	else
	{
		ret = _key_cache.FIFOBuffer[_key_cache.Read];
		if (++_key_cache.Read >= FIFOBUF_SIZE)
			_key_cache.Read = 0;
		return (ret);
	}
}
...
void SOLGUI_Menu_PageStage(void) //【非阻塞】SOLGUI前台页面切换器
{
	...
	if (cur_key == SOLGUI_KEY_BACK && SOLGUI_CSR == 0)
	{											   //检查是否为返回键值且CSR无占用
