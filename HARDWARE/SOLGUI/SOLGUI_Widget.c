...
void SOLGUI_Cursor(u8 rowBorder_Top, u8 rowBorder_Bottom, u8 option_num) //光标（上边界行，下边界行，选项个数）
{
	...
	if (option_num > OPTIONS_MAX)
		option_num = OPTIONS_MAX;							   //超过最大设为最大
															   //--------【相关参数计算】
	cursor->row_start = rowBorder_Top;						   //设置选项起始行
	cursor->viewport_width = rowBorder_Top - rowBorder_Bottom; //视口行数计算
	if (option_num <= cursor->viewport_width)
		cursor->viewport_width = option_num - 1; //选项数小于视口宽度处理
												 //--------【键值解析】
	cur_key = SOLGUI_GetCurrentKey();			 //获取当前全局键值
	if (cur_key == SOLGUI_KEY_UP)
	{ //当前全局为上键
		if (cursor->cursor_rel_offset > 0 && SOLGUI_CSR == 0)
			cursor->cursor_rel_offset--; //光标在视口内上移
		else if (cursor->viewport_offset > 0 && SOLGUI_CSR == 0)
			cursor->viewport_offset--; //视口上移
	}
	else if (cur_key == SOLGUI_KEY_DOWN)
	{ //当前全局为下键
		if ((cursor->cursor_rel_offset < cursor->viewport_width) && SOLGUI_CSR == 0)
			cursor->cursor_rel_offset++; //光标在视口内下移
		else if ((cursor->viewport_offset < option_num - cursor->viewport_width - 1) && SOLGUI_CSR == 0)
			cursor->viewport_offset++; //视口下移
	}
	...
	SOLGUI_printf(0, cursor_row, CUSTOMER_FONT, "%c", ICON_CURSOR);					 //光标绘制
																			 //---【上下边线】
	if (rowBorder_Top != 6)
		SOLGUI_GBasic_Line(0, cursor->y_t + 8, SCREEN_X_WIDTH - 1, cursor->y_t + 8, DOTTED); //上边线
	if (rowBorder_Top != 0)
		SOLGUI_GBasic_Line(0, cursor->y_b - 1, SCREEN_X_WIDTH - 1, cursor->y_b - 1, DOTTED);			   //下边线
																										   //---【滚动条】（可以作为选择添加的部分）
...
void SOLGUI_Widget_GotoPage(u8 USN, MENU_PAGE *page) //页面跳转控件
{
	...
	if (True == _OptionsDisplay_Judge(USN))
	{
		...
		//--------【选项绘制】
		SOLGUI_printf(6, y_disp, CUSTOMER_FONT, "%s", page->pageTitle);						  //下一个页面标题
		SOLGUI_printf(SCREEN_X_WIDTH - 13, y_disp, CUSTOMER_FONT, "%c", ICON_WIDGET_GOTOPAGE); //图标
																					  //--------【操作提示绘制】
		if ((cursor->viewport_offset + cursor->cursor_rel_offset) == USN)
			SOLGUI_printf(SCREEN_X_WIDTH - 6, 56, CUSTOMER_FONT, "%c", ICON_OK); //操作指示（根据键值解析部分来编写）
	}
}
...
void SOLGUI_Widget_Spin(u8 USN, const u8 *name, u8 type, double max, double min, void *value) //数字旋钮控件
{
	...
	if (True == _OptionsDisplay_Judge(USN))
	{
		...
		if (option_enable_list[USN] == 1)
		{
			...
			if (bit_istrue(SOLGUI_CSR, bit(0))) //如果处于SPIN占用中
			{
				...
				if (cur_key == SOLGUI_KEY_UP) //数值加步进
				{
					...
					case INT8:
					{
						if (((*v_i8) + dat_step) <= max)
							(*v_i8) += (s8)dat_step;
					}
					break;
					case UINT8:
					{
						if (((*v_u8) + dat_step) <= max)
							(*v_u8) += (u8)dat_step;
					}
					break;
					case INT32:
					{
						if (((*v_i32) + dat_step) <= max)
							(*v_i32) += (s32)dat_step;
					}
					break;
					case FLT16:
					{
						if (((*v_f16) + dat_step) <= max)
							(*v_f16) += (float)dat_step;
					}
					break;
					case FLT32:
					{
						if (((*v_f32) + dat_step) <= max)
							(*v_f32) += (double)dat_step;
					}
					break;
					case INT16:;
					default:
					{
						if (((*v_i16) + dat_step) <= max)
							(*v_i16) += (s16)dat_step;
					}
					break;
					}
				...
				else if (cur_key == SOLGUI_KEY_DOWN) //数值减步进
				{
					...
					case INT8:
					{
						if (((*v_i8) - dat_step) >= min)
							(*v_i8) -= (s8)dat_step;
					}
					break;
					case UINT8:
					{
						if (((*v_u8) - dat_step) >= min)
							(*v_u8) -= (u8)dat_step;
					}
					break;
					case INT32:
					{
						if (((*v_i32) - dat_step) >= min)
							(*v_i32) -= (s32)dat_step;
					}
					break;
					case FLT16:
					{
						if (((*v_f16) - dat_step) >= min)
							(*v_f16) -= (float)dat_step;
					}
					break;
					case FLT32:
					{
						if (((*v_f32) - dat_step) >= min)
							(*v_f32) -= (double)dat_step;
					}
					break;
					case INT16:;
				...
				{
					if (spin_pos < SPIN_DIGIT_MAX - 1)
						spin_pos++;
				}
				else if (cur_key == SOLGUI_KEY_RIGHT) //右步进
				{
					if ((type == FLT16) || (type == FLT32))
					{
						if (spin_pos > SPIN_DIGIT_MIN)
							spin_pos--;
					} //小数
					else
					{
		...
		{
		case INT8:
			SOLGUI_printf(68, y_disp, CUSTOMER_FONT, "%d", *v_i8);
			break;
		case UINT8:
			SOLGUI_printf(68, y_disp, CUSTOMER_FONT, "%d", *v_u8);
			break;
		case INT32:
			SOLGUI_printf(68, y_disp, CUSTOMER_FONT, "%d", *v_i32);
			break;
		case FLT16:
			SOLGUI_printf(68, y_disp, CUSTOMER_FONT, "%f", *v_f16);
			break;
		case FLT32:
			SOLGUI_printf(68, y_disp, CUSTOMER_FONT, "%f", *v_f32);
			break;
		case INT16:;
		default:
			SOLGUI_printf(68, y_disp, CUSTOMER_FONT, "%d", *v_i16);
			break;
		}

		//--------【旋钮绘制】
		if (bit_istrue(SOLGUI_CSR, bit(0)) && (option_enable_list[USN] == 1))
			SOLGUI_printf(0, y_disp, ~CUSTOMER_FONT, "%c%f%c", ICON_LEFT, dat_step, ICON_RIGHT); //如果处于设置模式中，显示旋钮数值
		else
			SOLGUI_printf(6, y_disp, CUSTOMER_FONT, "%s", name); //变量名
														//--------【操作提示绘制】
		if ((cursor->viewport_offset + cursor->cursor_rel_offset) == USN)
			SOLGUI_printf(SCREEN_X_WIDTH - 6, 56, CUSTOMER_FONT, "%c", ICON_OK); //操作指示（根据键值解析部分来编写）
	}
...
void SOLGUI_Widget_OptionText(u8 USN, const u8 *str, ...) //选项文本
{
	...
	if (True == _OptionsDisplay_Judge(USN))
	{
		...
		//--------【选项绘制】
		va_start(ap, str);
		__SOLGUI_printf(6, y_disp, CUSTOMER_FONT, str, ap); //文本显示
		va_end(ap);
	}
}
...
void SOLGUI_Widget_Button(u8 USN, const u8 *name, void (*func)(void)) //按键
{
	...
	if (True == _OptionsDisplay_Judge(USN))
	{
		...
		//--------【选项绘制】
		SOLGUI_printf(6, y_disp, CUSTOMER_FONT, "%s", name); //选项名
		SOLGUI_printf(68, y_disp, CUSTOMER_FONT, "->");		//图标
													//--------【运行牵连函数】
		...
		//--------【操作提示绘制】
		if ((cursor->viewport_offset + cursor->cursor_rel_offset) == USN)
			SOLGUI_printf(SCREEN_X_WIDTH - 6, 56, CUSTOMER_FONT, "%c", 0x84); //操作指示（根据键值解析部分来编写）
	}
}
...
void SOLGUI_Widget_Switch(u8 USN, const u8 *name, u32 *mem_value, u8 L_shift) //变量开关（mem_value是一个外部申请的非易失性存储器变量）
{
	...
	if (bit_istrue(SOLGUI_CSR, bit(1)))
		return; //全屏占用
				//--------【参数过滤】
	if ((L_shift < 1) || (L_shift > 32))
		L_shift = 1; //小于1或大于32都划归为1
	if (USN >= OPTIONS_MAX)
		return; //USN只允许取0~OPTIONS_MAX-1
				//--------【计算显示行】
	if (True == _OptionsDisplay_Judge(USN))
	{
		...
		SOLGUI_printf(6, y_disp, CUSTOMER_FONT, "%s", name); //选项名
													//--------【图标绘制】
		if (bit_istrue((*mem_value), temp))
			SOLGUI_printf(68, y_disp, CUSTOMER_FONT, "[ON ]"); //"开"文字
		else
			SOLGUI_printf(68, y_disp, CUSTOMER_FONT, "[OFF]"); //"关"文字
													  //--------【操作提示绘制】
		if ((cursor->viewport_offset + cursor->cursor_rel_offset) == USN)
			SOLGUI_printf(SCREEN_X_WIDTH - 6, 56, CUSTOMER_FONT, "%c", 0x84); //操作指示（根据键值解析部分来编写）
	}
...
{
	if (strlen((const char *)sour) > dest_size)
		strncpy((char *)dest, (char *)sour, dest_size);
	else
	...
	if (True == _OptionsDisplay_Judge(USN))
	{
		...
		if (option_enable_list[USN] == 1)
		{
			...
			if (bit_istrue(SOLGUI_CSR, bit(1))) //如果处于EDIT占用状态下
			{
				...
				else if (cur_key == SOLGUI_KEY_DOWN) //光标向下&字符减
				{
					...
					else if (bit_isfalse(SOLGUI_CSR, bit(2)) && edit_cursor_col < (SCREEN_Y_PAGE - 4) && ((buf_i + (SCREEN_X_PAGE - 2)) < edit_buf_size_temp))
						edit_cursor_col++; //纵光标加
				}
				else if (cur_key == SOLGUI_KEY_LEFT) //光标向左&字符类型左切换
				{
					if (bit_istrue(SOLGUI_CSR, bit(2)) && (SOL_ASCII_IME->finger > 0))
						SOL_ASCII_IME->finger--; //输入法换簇左
					else if (bit_isfalse(SOLGUI_CSR, bit(2)) && edit_cursor_row > 0)
						edit_cursor_row--; //横光标减
				}
				else if (cur_key == SOLGUI_KEY_RIGHT) //光标向左&字符类型右切换
				{
					if (bit_istrue(SOLGUI_CSR, bit(2)) && (SOL_ASCII_IME->finger < SOL_ASCII_IME->size - 1))
						SOL_ASCII_IME->finger++; //输入法换簇右
		...
		if (bit_istrue(SOLGUI_CSR, bit(1)))
		{
			...
			if (bit_istrue(SOLGUI_CSR, bit(2)))											   //字符编辑状态
			{
				...
				SOLGUI_printf(2, 1, CUSTOMER_FONT, "%s", SOL_ASCII_IME->cluster[SOL_ASCII_IME->finger]->name);											   //字符簇名
				SOLGUI_printf(64, 1, CUSTOMER_FONT, "[ %c ]", SOL_ASCII_IME->cluster[SOL_ASCII_IME->finger]->table[SOL_ASCII_IME->cluster[SOL_ASCII_IME->finger]->finger]);
			}
			else if ((edit_cursor_col == 0) && (edit_cursor_row == 0)) //底栏显示
			{
				SOLGUI_printf(2, 1, CUSTOMER_FONT, "BACK"); //光标指向back位置
			}
			else
			{
				SOLGUI_printf(2, 1, CUSTOMER_FONT, "L:%d C:%d", edit_cursor_col + 1, edit_cursor_row + 1); //光标指向可编辑字符位置
			}
			//----[内容显示]
				...
				for (j = 0; j < (SCREEN_X_PAGE - 2); j++)
				{
					if (i == 0 && j == 0)
					{
						...
						if ((edit_cursor_row == 0) && (edit_cursor_col == 0))
							SOLGUI_printf(6, 48, ~CUSTOMER_FONT, "%c", ICON_BACK); //确认操作指示，字符坐标(0,0)反白
						else
							SOLGUI_printf(6, 48, CUSTOMER_FONT, "%c", ICON_BACK);
					}
					else
					{
						//----[其他字符显示]
						...
						if (buf_i < edit_buf_size_temp)
						{
							...
							{
								if (edit_buf[buf_i] == '\0')
									SOLGUI_printf(6 + j * 6, 48 - i * 8, ~CUSTOMER_FONT, "%c", ICON_OTHER_HIDE); //'\0'用'...'来表示，即可编辑区域
								else
									SOLGUI_printf(6 + j * 6, 48 - i * 8, ~CUSTOMER_FONT, "%c", edit_buf[buf_i]); //选中的字反白显示
							}
							else
							{
								if (edit_buf[buf_i] == '\0')
									...
									SOLGUI_printf(6 + j * 6, 48 - i * 8, CUSTOMER_FONT, "%c", edit_buf[buf_i]);
							}
						}
						else
						{
							i = SCREEN_Y_PAGE;
							j = SCREEN_X_PAGE; //跳出循环
						}
					}
				}
			}
		...
		else
		{
			...
			SOLGUI_printf(68, y_disp, CUSTOMER_FONT, "%s", edit_thumbnail_buf);		  //缩略内容显示
			if (strlen((const char *)buf) > (SCREEN_X_PAGE - 14))
				SOLGUI_printf(SCREEN_X_WIDTH - 19, y_disp, CUSTOMER_FONT, "%c", ICON_OTHER_HIDE); //内容隐藏图标
			SOLGUI_printf(SCREEN_X_WIDTH - 13, y_disp, CUSTOMER_FONT, "%c", ICON_WIDGET_EDIT);	 //EDIT图标
		}
		//--------【操作提示绘制】
		if ((cursor->viewport_offset + cursor->cursor_rel_offset) == USN)
			SOLGUI_printf(SCREEN_X_WIDTH - 6, 56, CUSTOMER_FONT, "%c", ICON_OK); //操作指示（根据键值解析部分来编写）
...
void SOLGUI_Widget_Spectrum(u32 x0, u32 y0, u32 xsize, u32 ysize, s32 max, s32 min, u16 val_num, s32 value[])
{
	...
	for (i = 0; i < xsize; i++)
	{
		...
		x_now = x0 + i;
		if ((val_now >= min) && (val_now < max))
			y_now = y0 + (u32)((val_now - min) * delta_y);
		else if (val_now < min)
			y_now = y0;
		else
			y_now = y0 + ysize - 1;
...
void SOLGUI_Widget_Picture(u32 x0, u32 y0, u32 xsize, u32 ysize, const u8 *pic, u32 x_len, u32 y_len, u8 mode) //带缩小适配的图片控件
{
	...
	{
		//------------【图片适配大小绘制】
		fw = (float)x_len / xsize;
		fh = (float)y_len / ysize; //缩小比
		for (x_i = 0; x_i < xsize; x_i++)
		{
			...
			{
				temp_y = (u16)(fh * y_i);
				if (SOLGUI_GetPixel(temp_x, temp_y, pic, x_len, y_len))
					SOLGUI_DrawPoint(x0 + x_i, y0_t - y_i, m1);
				else
					SOLGUI_DrawPoint(x0 + x_i, y0_t - y_i, !m1);
			}
		}
